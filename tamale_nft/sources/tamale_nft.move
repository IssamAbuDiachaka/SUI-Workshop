module tamale_nft::tamale_nft {
    ...
    public struct Mintnft_event has copy, drop {
     ...
     ///
     /// Args:
     /// * `witness`: A TAMALE NFT.
     /// * `ctx`: The transaction context.
     fun init(witness: TAMALE_NFT, ctx: &mut TxContext) {
        ...
        let values = vector[
           ...
           // string::utf8(b"{url}"),
            string::utf8(b"{image_url}"),
            string::utf8(b"Tamale")
        ];

        let mut display = display::new_with_fields<Tamale_nft>(
            &publisher,
            keys,
            values,
            ctx
   ...
   entry fun mint(
    ...
    public entry fun burn(nft: Tamale_nft) {
        let Tamale_nft { id, name: _, description: _,  image_url: _ } = nft;
        object::delete(id);
    }
}
